...
def generate_openscad_code(user_query: str) -> str:
    ...
    # Only prepend use statements for needed files IF the code actually calls a module/function from that file
    # For simple primitives (cube, sphere, cylinder, etc.), do not add any use lines
    simple_primitives = ['cube', 'sphere', 'cylinder', 'polyhedron', 'polygon', 'square', 'circle']
    code_lower = code.lower()
    if any(primitive in code_lower for primitive in simple_primitives) and not any(sym in code_lower for sym in symbol_to_file):
        use_block = ''
    else:
